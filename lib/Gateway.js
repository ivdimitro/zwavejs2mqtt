...
function onNodeStatus (node) {
  ...
  // Publish availabity status to home assistant
  if (node.ready) {
    this.mqtt.publish(nodeTopic + 'hass/status', 'online')
  } else {
    this.mqtt.publish(nodeTopic + 'hass/status', 'offline')
  }
}
 ...
 * @param {any} valueId The valueId object
 ...
 *
 * @param {any} node The Zwave Node Object
 ...
 *
 * @param {any} node A Zwave Node Object
 ...
 *
 * @param {any} hassDevice The Hass device object configuration
 * @param {string} nodeName Node name from getNodeName function
...
Gateway.prototype.discoverValue = function (node, vId) {
  ...
  try {
    ...
    if (payload.availability_topic === true) {
      payload.availability_topic = setTopic || getTopic + '/status/hass'
    }
